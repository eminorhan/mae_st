...
def eval_video_tracking_davis(args, model, frame_list, video_dir, first_seg, seg_ori, color_palette):
    ...
    # first frame
    frame1, ori_h, ori_w = read_frame(frame_list[0])  # first 16 frames

    # extract first frame feature
    frame1_feat = extract_feature(model, frame1).T   # pass first 16 frames through video model 
    ...
    for cnt in tqdm(range(1, len(frame_list))):
        ...
        # upsampling & argmax
        frame_tar_avg = F.interpolate(frame_tar_avg, scale_factor=14, mode='bilinear', align_corners=False, recompute_scale_factor=False)[0]
...
def extract_feature(model, frame, return_h_w=False):
    """Extract one frame feature everytime."""  # frame should be 1, c, t, h, w
    frame = frame.unsqueeze(1).repeat((1, 16, 1, 1))  # copy frame t=16 times
    frame = frame.unsqueeze(0)
    out = model.get_last_activation(frame.cuda())
    out = out[:, 1:, :].squeeze(0)  # we discard the [CLS] token
    h, w = int(frame.shape[-2] / 14), int(frame.shape[-1] / 14)
    dim = out.shape[-1]
    out = out.reshape(8, h, w, dim)
    out = out.mean(0)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ckpt', default='', type=str, help='Path to pretrained model weights to evaluate.')
    ...
    model = models_vit.vit_huge_patch14(img_size=args.img_size, num_classes=1000)
    ...
    for i, video_name in enumerate(video_list):
        ...
        first_seg, seg_ori = read_seg(seg_path, 14)
